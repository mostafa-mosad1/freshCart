  ...
  return (
    ...
    >
      <div className="container mx-auto flex items-center justify-between">
        ...
        {/* Right Side: Wishlist, Cart & Auth Actions (Always visible) */}
        <div className="flex items-center gap-2.5 sm:gap-3.5">
          {/* Wishlist Link & Counter (Always visible) */}
          <Link
            to="/WishList"
            className="relative p-2.5 text-slate-600 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all duration-200"
            title="Wishlist"
          >
          ...
          {/* Cart Link & Counter (Always visible) */}
          <Link
            to="/cart"
            className="relative flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-3.5 py-2 rounded-xl transition-all duration-200 shadow-sm shadow-emerald-500/20 active:scale-95"
            title="Shopping Cart"
          >
          ...
          {token ? (
            ...
            <button
              onClick={Logout}
              className="hidden lg:flex items-center gap-1.5 text-slate-500 hover:text-rose-600 font-medium text-sm px-3 py-2 rounded-xl hover:bg-rose-50/80 transition-all border border-transparent hover:border-rose-100"
              title="Log Out"
            >
            ...
            /* Auth Buttons for Guests (Desktop) */
            <div className="hidden lg:flex items-center gap-2 pl-2 border-l border-slate-200">
              <NavLink
                to="/login"
                className={({ isActive }) =>
                  isActive
                    ? "text-emerald-600 font-semibold px-3.5 py-1.5 rounded-xl bg-emerald-50 border border-emerald-100 text-sm"
                    : "text-slate-600 hover:text-emerald-600 font-medium px-3.5 py-1.5 rounded-xl hover:bg-slate-50 text-sm transition-all"
                }
              >
                Sign In
              </NavLink>
              <NavLink
                to="/register"
                className="bg-slate-900 hover:bg-slate-800 text-white font-medium text-sm px-3.5 py-1.5 rounded-xl shadow-xs transition-all active:scale-95"
              >
                Register
              </NavLink>
            </div>
        ...
        <div className="lg:hidden bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 py-5 shadow-lg animate-in slide-in-from-top-2 duration-200">
          <div className="flex flex-col gap-2">
            ...
            <NavLink
              to="/cart"
              onClick={() => setOpen(false)}
              className="flex items-center justify-between text-slate-700 hover:text-emerald-600 font-medium px-4 py-2.5 rounded-xl hover:bg-slate-50 text-sm transition-all"
            >
